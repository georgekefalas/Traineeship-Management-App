...
@Service("interestBasedStrategy")
public class InterestBasedSearchStrategy implements TraineeshipSearchStrategy {
    ...
    public List<TraineeshipPosition> findMatchingPositions(Student student) {
        ...
        return allAvailable.stream()
            .filter(position -> studentMatchesSkills(studentSkills, position.getSkills().stream()
            		.map(String::toLowerCase)
        		    .collect(Collectors.toList())))
            .filter(position -> {
